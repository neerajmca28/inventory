...
$to_installer=$_GET['to_installer'];
...
if($count>0)
{
				...
				//  $pdf->cell(18,10,'',0);
				  ...
				  $pdf->cell(70,8,'',0);
				  $pdf->cell(100,8,'Recevied Remove Devices Challan',0);
				  		   
				  $pdf->ln(15);
				  $pdf->SetFont('Arial','',11);
				  $pdf->cell(50,6,'Dispatch Address :',0);
				 $pdf->cell(50,6,$GetChallanDetailByNo[0]['branch_name'],0);
				  $pdf->cell(50,6,'Delivered By:',0);
				  $pdf->cell(50,6,$login_name,0);
			 
					
					// Column headings
					$header = array('S.no', 'ITGC ID', 'IMEI','Antena','Immob', 'Conn', 'Assign Remarks', 'InstallerName');
					$pdf->SetFont('Arial','',8);
					//print_r($header); die;
					...
					$pdf->Ln(15);
					// Header
					$w = array(10,30,30,10,10,10,55,30);
							//print_r($w); die;
				...
				for($j=0;$j<$count;$j++)
				{
					...
					$pdf->Cell($w[3],6,$GetChallanDetailByNo[$j]['DispatchAntennaCount'],'LR',0,'C',$fill);
					$pdf->Cell($w[4],6,$GetChallanDetailByNo[$j]['DispatchImmobilizerCount'],'LR',0,'C',$fill);
					$pdf->Cell($w[5],6,$GetChallanDetailByNo[$j]['DispatchConnectorCount'],'LR',0,'C',$fill);
					$pdf->Cell($w[6],6,$GetChallanDetailByNo[$j]['device_installer_remarks'],'LR',0,'C',$fill);
					$pdf->Cell($w[7],6,$GetChallanDetailByNo[$j]['InstallerName'],'LR',0,'C',$fill);
					$pdf->Ln();
					$fill = !$fill;
				}
		// Closing line
