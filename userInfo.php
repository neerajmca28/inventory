...
if(isset($_GET['action']) && $_GET['action']=='vendor_device_addition')
{ 
   $count=0;
   $flag=0;
   $errorMsg="";
   ...
   $is_vendor = $_GET['gtrac'];
   $vendor_name=$_SESSION['user_name_inv'];
   $no_of_device=$_POST['no_of_device'];
   $GetVendorName=select_Procedure("CALL GetVendorName('".$vendor_name."')");
   $vendor_id=$GetVendorName[0][0][0]; 
 ...
 if($flag==0) 
  {
   for($i=0;$i<$no_of_device;$i++)
   {
    ...
    {
     $checkSno=$masterObj->checkSno($txtVisionTekId[$i]);
     if($checkSno>0)
     {
      $flag=5;
      break;
    ...
    {
     if(!(preg_match ("/^([0-9]+)$/", $sim_no[$i]))) 
     {
      $flag=7;
      break;
     }
     else if(strlen($sim_no[$i])>19 || strlen($sim_no[$i])<1)
     {
      
      $flag=8;
      break;
     }
     $check_simno=$masterObj->check_simno($sim_no[$i]);
     if($check_simno>0)
     {
      $flag=15;
      break;
    ...
    {
     if(!(preg_match ("/^([0-9]+)$/", $phone_no[$i]))) 
     {
      $flag=9;
      break;
     }
     else if(strlen($phone_no[$i])>10 || strlen($phone_no[$i])<10)
     {
      
      $flag=10;
      break;
     }
     $check_phone=$masterObj->phone_no($phone_no[$i]);
     if($check_phone>0)
     {
      $flag=16;
      break;
    ...
    if(!empty($imei_no[$i]) || $imei_no[$i]!="")
    {
     ...
     $checkimei=$masterObj->check_imei($imei_no[$i]);
     if($checkimei>0)
     {
      $flag=14;
      break;
    ...
    if($txtVisionTekId[$i]=="" ||  $sim_no[$i]=="" || $imei_no[$i]=="" || $phone_no[$i]=="")
    {
     $flag=13;
     break;
    } 
  ...
  if($flag==1)
  {
   echo $errorMsg="Enter the Different Sno in each Textbox.";
  }
  else if($flag==2)
  {
   echo $errorMsg="Enter the Different Sim No in each Textbox.";
  }
  else if($flag==3)
  {
   echo $errorMsg="Enter the Different Phone No in each Textbox.";
  }
  else if($flag==4)
  {
   echo $errorMsg="Enter the Different IMEI in each Textbox.";  
  }
  else if($flag==5)
  {
   echo $errorMsg="Sno Aleady Exist.";
  } 
  else if($flag==6)
  {
   echo $errorMsg="Please Insert Numeric Sno.";
  }
  else if($flag==7)
  {
   echo $errorMsg="Please Insert Numeric Sim No.";
  }  
  else if($flag==8)
  {
   echo $errorMsg="Please Insert Sim No between 0 to 20.";
  } 
  else if($flag==9)
  {
 
   echo $errorMsg="Please Enter Numeric Phone No.";
  } 
  else if($flag==10)
  {

   echo $errorMsg="Please Enter 10 Digit Phone No.";
  }
  else if($flag==11)
  {
   echo $errorMsg="Please Enter Numeric IMEI No.";
  }
  else if($flag==13)
  {
   echo $errorMsg="Please Fill All Fields";
  }
  else if($flag==14)
  {
  ...
  {
  echo $errorMsg="Sim No. Already Exist.";
  }
  else if($flag==16)
  {
   echo $errorMsg="Phone No. Already Exist.";
  }
  else
  {
  ...
  if($errorMsg=="" && $errNoDevice=="" && $errdevice_type=="" && $errmodel_type=="")
  {
   for ($i = 0; $i < $no_of_device; $i++) 
   {
    ...
    $sim_no1=$_GET['sim_no'][$i];
    $phone_no1=$_GET['phone_no'][$i];
    $txtimei=$_GET['imei_no'][$i];
    $oprt= $operator[$i]; 
    //echo "CALL InsertDeviceVendor('".$device_sno."', '".$txtimei."', '".$model_type."','".$recd_date."','".$device_type."','".$is_vendor."','".$sim_no1."','".$phone_no1."','".$oprt."','".$vendor_id."','".$branch_id."')"; die;
    $tt= select_Procedure("CALL InsertDeviceVendor('".$device_sno."', '".$txtimei."', '".$model_type."','".$recd_date."','".$device_type."','".$is_vendor."','".$sim_no1."','".$phone_no1."','".$oprt."','".$vendor_id."','".$branch_id."')");
    if(count($tt)>0)
    {
     $flag=10;
    }
...
if(isset($_GET['action']) && $_GET['action']=='CrackAdd')
{	
			//echo '<pre>'; print_r($_GET);'</pre>'; die;
			//$installer_id = $_GET['installer_list_hidden']; 
			$device_type = $_GET['device_type']; 
			$model_type=$_GET['model_type'];
			$service_req = $_GET['service_req']; 
			$device_imei = $_GET['device_imei'];
			$ser_req_id = $_GET['ser_req_id'];
			$ser_req_crack_id = $_GET['ser_req_crack_id'];
			$sim_no=$_GET['sim_no'];
			$recd_date=$_GET['date'];
			$txtVisionTekId=$_GET['txtVisionTekId'];
			//$installer_id=$_GET['installer_list'];
			//print_r($txtVisionTekId); die;
			$serial_no=$_GET['serial_no'];
			// $sim_no=$_GET['sim_no'];
			$itgcid=$serial_no[0].'X'.$txtVisionTekId[0];
			
			// if($_GET['crack_dev']=='gtrac_device')
			// {
			// 	$check_gtrac=0;
			// }
			// if($_GET['crack_dev']=='online_crack')
			// {
			// 	$check_gtrac=1;
			// }
			$check_gtrac=1;
			...
			$flag=0;
				...
				if(!empty($txtVisionTekId[0]) || $txtVisionTekId[0]!="")
				{
					//echo 'ss';
					$checkIMEI=$masterObj->checkSno($txtVisionTekId[0]);
					if($checkIMEI>0)
					{
						$flag=2;
						
					}
					else if(!(preg_match ("/^([0-9]+)$/", $txtVisionTekId[0]))) 
					{
						$flag=5;
				
					}
					else if(strlen($txtVisionTekId[0])>16 || strlen($txtVisionTekId[0])<1)
					{
			    ...
			    // if(!empty($sim_no) || $sim_no!="")
			    // {
			    //  if(!(preg_match ("/^([0-9]+)$/", $sim_no))) 
			    //  {
			    //   $flag=7;
			     
			    //  }
			    //  else if(strlen($sim_no)>19 || strlen($sim_no)<1)
			    //  {
			    ...
			    //  // $check_simno=$masterObj->check_simnoForRND($sim_no);
			    //  // if($check_simno>0)
			    //  // {
			    //  //  	$flag=15;
			    ...
			    //  // } 
			    // }
			    if(!empty($device_imei) || $device_imei!="")
			    {
			    	$check_IMEI=$masterObj->checkIMEI($device_imei);
			    	 if($check_IMEI>0)
			    	 {
			      		$flag=16;
			     	 }
			     	 else if(!(preg_match ("/^([0-9]+)$/", $device_imei))) 
					{
						$flag=17;
	 	...
	 	if($flag==2)
		{
			echo $errorMsg="Device ID Already Exist";
		}
		else if($flag==4)
		{
		...
		{
			echo $errorMsg="Please Insert Numeric Device ID.";
		}
		// else if($flag==7)
  // 		{
  //  			echo $errorMsg="Please Insert Numeric Sim No.";
  ...
  // 		} 
  		// else if($flag==15)
  		// {
  		// 	echo $errorMsg="Please Contact to Sim Dispatch Team.";
  		// }
  		else if($flag==17)
 		{
  			 echo $errorMsg="Please Insert Numeric IMEI.";
  		} 
  		else if($flag==16)
  		{
  			 echo $errorMsg="IMEI Already Exist.";
		...
		if($errorMsg=="" && $errNoDevice=="" && $errdevice_type=="" && $errmodel_type=="")
		{
				...
				//$txtimei="";
				// echo "CALL inserOnlineCrackDevice('".$txtVisionTekId[0]."','".$device_imei."', '".$model_type."','".$device_type."','".$recd_date."','".$check_gtrac."','".$username_client."','".$itgcid."','".$serial_no[0]."','".$sim_no."','".$installer_id."')"; die;
				//echo "CALL insertCrackDevice('".$txtVisionTekId[0]."','".$device_imei."', '".$model_type."','".$device_type."','".$recd_date."','".$check_gtrac."','".$username_client."','".$itgcid."','".$serial_no[0]."','".$sim_no."')"; die;
				$tt= select_Procedure("CALL insertCrackDevice('".$txtVisionTekId[0]."','".$device_imei."', '".$model_type."','".$device_type."','".$recd_date."','".$check_gtrac."','".$username_client."','".$itgcid."','".$serial_no[0]."','".$sim_no."')");
				if(count($tt)>0)
				{
					// $query=db__update_staging("update internalsoftware.installation set inv_online_crack_sts=1 where id='".$install_req."' ");
					$data = array('service_status' => 203);
					$condition = array('id' => $ser_req_id );
					$condition1 = array('id' => $ser_req_crack_id );
					$query=update_query('internalsoftware.services',$data,$condition);
					$query=update_query('internalsoftware.services_crack',$data,$condition1);
					//echo $query; die;
					$flag=10;
				}
				else
...
if(isset($_GET['action']) && $_GET['action']=='onlineCrackAdd')
{	
			//echo '<pre>'; print_r($_GET);'</pre>'; die;
			$installer_id = $_GET['installer_list_hidden']; 
			$device_type = $_GET['device_type']; 
			$model_type=$_GET['model_type'];
			$install_req = $_GET['install_req']; 
			$device_imei = $_GET['device_imei'];
			$sim_no=$_GET['sim_no'];
			$recd_date=$_GET['date'];
			$txtVisionTekId=$_GET['txtVisionTekId'];
			...
			if($_GET['online_crack']=='gtrac_device')
			{
				$check_gtrac=0;
			}
			if($_GET['online_crack']=='online_crack')
			{
				$check_gtrac=1;
			}
			if($device_type==0)
		...
		if($errorMsg=="" && $errNoDevice=="" && $errdevice_type=="" && $errmodel_type=="")
		{
				...
				$tt= select_Procedure("CALL inserOnlineCrackDevice('".$txtVisionTekId[0]."','".$device_imei."', '".$model_type."','".$device_type."','".$recd_date."','".$check_gtrac."','".$username_client."','".$itgcid."','".$serial_no[0]."','".$sim_no."','".$installer_id."')");
				if(count($tt)>0)
				{
					
					// $query=db__update_staging("update internalsoftware.installation set inv_online_crack_sts=1 where id='".$install_req."' ");
					$data = array('inv_online_crack_sts' => 1);
					$condition = array('id' => $install_req );
					$query=update_query_internal('internalsoftware.installation',$data,$condition);
					//echo $query; die;
					$flag=10;
...
if(isset($_GET['action']) && $_GET['action']=='configure_device')
{
	$errMsg="";
	//print_r($_GET);die;
		//echo "tt"; die;
		for($i=0;$i<count($_GET['rowVal']);$i++)
		{
			//echo $_GET['imei'][$i]; die;
			for($j=$i+1;$j<count($_GET['rowVal']);$j++)
			{
				$imei_no=$_GET['imei'][$i];
				//echo $_GET['imei'][$i];
				//echo $_GET['imei'][$j]; die;
				if($_GET['imei'][$i]==$_GET['imei'][$j]) 
				{
					$flag=1; 
					break;
				}
			}	
		   if($_GET['imei'][$i]=="" ||  empty($_GET['imei'][$i]))
			{
				$flag=3;
				break;
			}			
			else if(!preg_match ("/^([0-9]+)$/", $_GET['imei'][$i]))
			{
				$flag=2;
				break;
			}
			else if(!empty($_GET['imei'][$i]) || $_GET['imei'][$i]!="")
			{
				$checkImei=$masterObj->getImei($_GET['imei'][$i]);
				if($checkImei>0)
				{
					$flag=6;
					break;
	  ...
	  if($flag==1)
	  {
		  echo $errMsg="Please enter different IMEI in all selected rows. ";
	  }
	  if($flag==2)
	  {
	  ...
	  if($flag==3)
	  {
		  echo $errMsg="Please enter All IMEI";
	  }
	  if($flag==6)
	  {
		  echo $errMsg="IMEI is Already Exist.";
	  }
		...
		if($errorSimNo=="" )
		{
			for($i=0;$i<$no_of_sim;$i++)
			{
				 ...
				 else
				 {
					...
					$oprt= $operater[$i]; 
					//echo "CALL InsertSim('".$sim_no[$i]."', '".$phone_no[$i]."', '".$oprt."','".$sim_status."')"; die;
...
if(isset($_GET['count'])&& $_GET['action']=='alert_notification')
{
		$select_Notification=$masterObj->select_Notification(); 
		echo $select_Notification[0]['no_of_notification']; 
		//echo "<pre>";print_r($select_Notification[0]['no_of_notification']);"</pre>"; 
		//die;
}
