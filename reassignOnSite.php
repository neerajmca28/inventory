...
if (isset($_POST['submit']))
{
	...
	{				
		...
		$challan_mode=$Installer;
			...
			for($i=0;$i<count($_POST['rowVal']);$i++)
			{
				if(isset($_POST['rowVal'][$i]))
				{
					...
					$insertChallanDetailsDeadDeviceInstaller=$masterObj->insertChallanDetailsDeadDeviceInstaller2($deviceId,$AssignedAntennaCount,$AssignedImmobilizerType,$AssignedImmobilizerCount,$AssignedConnectorCount,$installer_list_to,$installer_list_from,$branchId,$Insaller_AssignDate);
			...
			function send_notification($tokens,$message,$androidkey)
			{
					...
					//echo "select * from internalsoftware.mobilekey where inst_id='".$installer_list_from."'"; die;
					$token_get = db__select_staging("select * from internalsoftware.mobilekey where inst_id='".$installer_list_from."'");
				    ...
				    if(count($token_get)>0)
				    {
				       ...
				       //echo "You Assigned ".$countDevice." Devices"; die;
				        ...
				        $Notificato_msg = array("title" => "Device Addition Request", "subtitle" => "Give ".$countDevice." Devices to ".$Installer_name_to, "installelrid" => $installer_list_from,"installelrid2" => "blank", "installelrName" => "blank","installelrName2" => "blank","branchid" => $branchId,"branchid2" => "blank", "imei" => $imei, "Msg" => "one");
      ...
      // 					</script>";	
      					 ?><script><?php echo("location.href = '".__SITE_URL."/reassignOnSite.php';");?></script><?php		
		}		
  ...
  <div class="col-12"> 
    ...
    <div class="portlet box yellow">
      <div class="portlet-title">
        <div class="caption"> <i class="fa fa"></i>Transfer To Installer </div>
      </div>
      <div class="portlet-body control-box">
         ...
         <option value="0">Select</option>
		   ...
		   <div class="right-left">Transfer To</div>
