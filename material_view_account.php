...
$viewAllMaterial_account=select_Procedure("CALL viewAllMaterial_account()");
$viewAllMaterial_account=$viewAllMaterial_account[0];
$rowcount=count($viewAllMaterial_account);
//echo '<pre>';print_r($viewAllMaterial_account); echo '</pre>'; die;
$errorMsg="";
$data=array();
if (isset($_POST['submit']))
{
		for($i=0;$i<count($_POST['rowVal']);$i++)
		{
			if(isset($_POST['rowVal'][$i]))
			{
			    ...
			    $material_id=$data[0];
			    $status=$admin_status;
			    // echo "CALL sendToAdminforApproval('".$material_id."','".$Remarks."','".$status."')"; die;
				 $viewAllMaterial=select_Procedure("CALL sendToAdminforApproval('".$material_id."','".$Remarks."','".$status."')");
			}		
		}
	?><script><?php echo("location.href = '".__SITE_URL."/material_view_account.php';");?></script><?php
}
  ...
  <div class="col-12"> 
    ...
    <div class="portlet box yellow">
      ...
      <div class="portlet-body">
        <table class="table table-bordered" id="filtertable">
          ...
          <tbody>
            ...
            <tr bgcolor="<?php echo $color; ?>" title="<?php echo $tool_tip ?>">
              <td><input type="checkbox" id="check<?php echo $y; ?>" name="rowVal[]" value="<?php echo $viewAllMaterial_account[$x]['material_id'];?>" onclick="setRow('<?php echo $y; ?>');" onclick="setRow('<?php echo $y; ?>');" class="checkbox1"></td>
				 <td><?php echo $viewAllMaterial_account[$x]['stock_remarks'];?></td>
			<!-- 	    <td><?php echo $viewAllMaterial_account[$x]['quant1'];?></td>
				 <td><?php echo $viewAllMaterial_account[$x]['subcategory'];?></td> -->
              <td>
        ...
        <tr>
              <td colspan="11"><input type="submit" class="btn btn-default table-btn-submit" name="submit" id="submit" value="Send To Admin"></td>
              <!--  <td colspan="11"><input type="submit" class="btn btn-default table-btn-submit" name="submit" id="submit" value="Back To Stock"></td> -->
         </tr>
