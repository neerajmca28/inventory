...
$inst_req_id=$_GET['inst_req_id'];
$request_type=$_GET['request_type'];
//echo "select inst_name,inst_id from installer where inst_id='".$inst_id."'"; die;
$installerList=db__select_staging("select inst_name,inst_id from installer where inst_id='".$inst_id."'");
//print_r($installerList); die;
// $simList=db__query("select sim.sim_no,sim.phone_no from sim inner join simchallandetail on sim.sim_id=simchallandetail.sim_id WHERE flag=1 and branch_id=1 and sim_status='93'and active_status=1 and RepairName=17");
//echo "select sim.sim_no,sim.phone_no from sim inner join simchallandetail on sim.sim_id=simchallandetail.sim_id WHERE sim.flag=1 and sim.active_status=1 and simchallandetail.installerID='".$inst_id."' "; die;
$simList=db__query("select sim.sim_no,sim.phone_no from sim inner join simchallandetail on sim.sim_id=simchallandetail.sim_id WHERE sim.flag=1 and sim.active_status=1 and simchallandetail.installerID='".$inst_id."' and simchallandetail.CurrentRecord=1 ");
//echo $simList; die;
  ...
  <div class="col-12"> 
    ...
    <div class="portlet box yellow">
      <div class="portlet-title">
        <div class="caption"> <i class="fa fa"></i>Add Online Crack Device </div>
      </div>
      <div class="portlet-body control-box">
        ...
        <div class="left-item"> <span>Device :</span> </div>
          ...
          <table>
            <tr>
              <td><input type="radio" name="online_crack" value="online_crack" checked></td>
              <td>Online Crack</td>
     ...
     <!--       <div class="content-box">
         ...
         <option>Select Model Type</option>
          ...
          <div class="right-item"> <input type="text" name="device_imei" id="device_imei" class="form-control" ></div>
         <span class="error">* <?php echo $errNoDevice;?></span> </div> 
          <!--  <div class="content-box">
         ...
         <option value="0">Select</option>
			...
			{
			?>
            		<option value="<?=$simList[$i]['sim_no']?>"><?php echo $simList[$i]['phone_no'];?>
            		</option>
            <? 
            } 
            ?>
		 ...
		 <input type="hidden" name="installer_list_hidden" id="installer_list_hidden" value="<?php echo $installerList[0]['inst_id']; ?>">
		  <input type="hidden" name="install_req" id="install_req" value="<?php echo $inst_req_id; ?>">
		  </div>

			<div class="content-box">
           <div class="left-item"> <span>Assign Installler:</span> </div>
          <div class="right-item"> <input type="text" name="installer_list" id="installer_list" class="form-control" value="<?php echo $installerList[0]['inst_name']; ?>" readonly></div>
         <span class="error">* <?php echo $errNoDevice;?></span> </div> 
           <div class="content-box">
 ...
 $form(document).ready(function () {
    ...
    $form('.btnaddsim').click(function (){
	 ...
	 $form('.submit').click(function (){
		...
		$form.ajax({
				type:"GET",
				url:"userInfo.php?action=onlineCrackAdd",
				...
				success:function(msg)
				{
					...
					{
					   document.location.href = 'addDevices_online_crack.php';
					}
