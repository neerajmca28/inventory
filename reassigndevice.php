	...
	{				
		...
		else
		{
			...
			function send_notification($tokens,$message,$androidkey)
			{
				...
				//echo "select * from internalsoftware.mobilekey where inst_id='".$selectInstallerTo."'";die;
					$token_get = db__select_staging("select * from internalsoftware.mobilekey where inst_id='".$installer_list_to."'");
				    ...
				    if(count($token_get)>0)
				    {
				       ...
				       //echo "You Assigned ".$countDevice." Devices"; die;
				        ...
				        $Notificato_msg = array("title" => "Device Addition Request", "subtitle" => "You have been Assigned ".$countDevice." New Devices", "installelrid" => $installer_list_to,"installelrid2" => "blank", "installelrName" => "blank","installelrName2" => "blank","branchid" => $branchId,"branchid2" => "blank", "imei" => $imei, "Msg" => "one");
