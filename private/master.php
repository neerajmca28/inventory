...
class master
{
		...
		function insertChallanDetailsDeadDeviceInstaller2($DeviceId,$strArrAntenna,$strArrImmobilerType,$strArrImmobilzerCount,$strArrConnectorCount,$installer_id,$installer_list_from,$branchId,$date_time)
		{
			$condition = "";
			//echo "UPDATE inventory.challandetail set TranferInstallerId='".$installer_id."' where  InstallerID='".$installer_list_from."' and DeviceID='".$DeviceId."' and CurrentRecord=1"; die; 
			$this->data=db__select("UPDATE inventory.challandetail set TranferInstallerId='".$installer_id."', ConfirmStatus='0' where  InstallerID='".$installer_list_from."' and DeviceID='".$DeviceId."' and CurrentRecord='1' ", $condition);
			return $this->data;
		
		}

	// function insertChallanDetailsDeadDeviceInstaller1($strChallanNo,$DeviceId,$strArrAntenna,$strArrImmobilerType,$strArrImmobilzerCount,$strArrConnectorCount,$installer_id,$installer_list_from,$branchId,$date_time)
	// 	{
			
	// 				  $condition = "";
	// 				  $insert_data="INSERT INTO challandetail(DispatchChallanNo,InstallerChallanNo,DeviceID,ChallanStatus,AssignedAntennaCount,AssignedImmobilizerType,AssignedImmobilizerCount,AssignedConnectorCount,CurrentRecord,InstallerID,TranferInstallerId,BranchID,AssignedDate) VALUES('".$strChallanNo."','".$strChallanNo."','".$DeviceId."',1,'".$strArrAntenna."','". $strArrImmobilerType."','".$strArrImmobilzerCount."','".$strArrConnectorCount."',1,'".$installer_id."','".$installer_list_from."','".$branchId."','".$date_time."')";
	...
	// 	}  
		...
		function check_imei($imei_no)
		{
		 ...
		 function check_simno($sim_no)
		  ...
		  function insert_Data($car_name,$year,$email,$phone_no)
		  {
		    $condition = "";
		    $insert_data="INSERT INTO car_sathi.tracking_car(car_name,year,email,phone_no) VALUES('".$car_name."','".$year."','".$email."','".$phone_no."')";
		    $this->data=db__select($insert_data,$condition);
		    return count($this->data);
			...
			{
					  $condition = "";
					 // echo "INSERT INTO SimChallanDetail(DispatchChallanNo,sim_id,BranchID,CurrentRecord,installerID,AssignRemarks,AssignDate,RepairName) VALUES('".$strChallanNo."','".$sim_id."','".$branchId."',1,'".$Installer_id."','". $remark."','". $AssignDate."','". $RepairName."')"; die;
			...
			function GetSimInstallerChallanDetailByNo($strChallanNo)
			 {
			    $condition = "";
			    //echo "select CD.RepairName,CD.InstallerID,CD.DispatchChallanNo,CD.sim_id,D.sim_no,D.branch_id,D.operator, D.phone_no from SimChallanDetail CD LEFT JOIN sim D ON CD.sim_id=D.sim_id  WHERE CD.DispatchChallanNo='".$strChallanNo."' AND CD.CurrentRecord=1"; die;
			...
			function selectChallanDetails($DeviceId)
			{
				$condition = "";

				$this->data=db__select( "select InstallerChallanNo as ChallanNo,DispatchChallanNo,BranchID,DispatchAntennaCount,DispatchImmobilizerType,DispatchImmobilizerCount,DispatchConnectorCount,DispatchDate from challandetail where DeviceID='".$DeviceId."' order by DispatchDate desc limit 1", $condition); 
				return $this->data;
...
//echo "select CD.InstallerName,D.device_installer_remarks,CD.BranchID, CD.DispatchChallanNo,CD.DeviceId,D.itgc_id,D.device_sno,D.device_imei,IM.item_name AS DeviceType,CD.DispatchAntennaCount,CD.DispatchImmobilizerType,CD.DispatchImmobilizerCount,CD.DispatchConnectorCount,branch_master.branch_name from ChallanDetail CD LEFT JOIN device D ON  CD.DeviceID=D.device_id LEFT JOIN item_master IM ON D.device_type=IM.item_id LEFT JOIN branch_master ON CD.BranchID=branch_master.id WHERE CD.DispatchChallanNo='".$strChallanNo."'";die;
		...
		{
			$condition = "";
			$this->data=db__select("select device_imei,itgc_id from device WHERE active_status=1 AND device_imei='".$new_imei."'", $condition);
			return count($this->data);
		}
		...
		{
			$condition = "";
			$this->data=db__select_staging("select inst_name from internalsoftware.installer WHERE inst_id='".$Installer_id."' AND is_delete=1", $condition);
			return $this->data;
	...
	function getImei($imei_no )
	  {
		...
		{
			$condition = "";
			$this->data=db__select("select internalsoftware.sys_device_id from services where veh_reg='".$strPhoneNo."'", $condition);
		...
		{
			$condition = "";
			$this->data=db__select("UPDATE devices set imei='".$strIMEI."' where id='".$strDeviceID."'", $condition);
     ...
     function newQryService($imei)
   {
    ...
    $this->data=db__select_matrix("select services.id,veh_reg from matrix.services left join matrix.devices on services.sys_device_id=devices.id where devices.imei = '".$imei."'", $condition); 
    ...
    return $this->data;
	 ...
	 //    {
	 //    	$condition = "";
		//     //echo "select inst_name from internalsoftware.installer where inst_id='".$InstallerId."'"; die;
		//     $this->data=db__select("select inst_name from internalsoftware.installer where inst_id='".$InstallerId."'", $condition);
		//     return $this->data;
	 //   }
