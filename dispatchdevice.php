...
$deviceData=select_Procedure("CALL SelectAttachedSim()");
		...
		else
		{
			...
			for($i=0;$i<count($_POST['rowVal']);$i++)
			{
				...
				if(isset($_POST['rowVal'][$i]))
				{
					...
					$dispatch_send=$selectDispatchBranch[0]['branch_name'];
						...
						else
						{
							...
							$SetDispatchDate1=select_Procedure("CALL SetDispatchDate1('".$DeviceId."','".$status1."','".$dispatch_medium."','".$dispatch_medium_remarks."','".$remark."','".$send_branch."','".$device_condition."','".$send_branch."','".$dispatch_date."')");
							$updateChallanDetails=$masterObj->updateChallanDetails($DeviceId);  
				...
				echo "<script type='text/javascript'>   
     				  window.open('challan_dispatch.php?challanNo=$strChallanNo&dispatch_send=$dispatch_send&deviceIDList=$arrDeviceIdStr');
      				 </script>";
      				 ?><script><?php echo("location.href = '".__SITE_URL."/dispatchdevice.php';");?></script><?php
			}
		}
...
<article> 
 <div class="col-12">
   ...
   <div class="color-sign" style="margin: 15px 0 2px 0;">
        <div class="cl-item"><span class="lightblue"></span><span>Repaired Device</span></div>
      <div class="cl-item"><span class="white"></span><span>New Device</span></div>
      <div class="cl-item"><span class="aqua"></span><span>Recevied from Branch</span></div>
      <div class="cl-item"><span class="red"></span><span>FFC Permanent</span></div>
  </div>
        ...
        <table class="table table-bordered" id="filtertable">
          ...
          <tbody>
            ...
            <tr bgcolor="<?php echo $color; ?>" title="<?php echo $tool_tip ?>">
              ...
              <td><input type="checkbox" <?php if ($is_permanent_ffc==1 && $is_ffc==0) echo 'disabled'; ?> id="check<?php echo $y; ?>" name="rowVal[]" value="<?php echo $deviceData[$x]['device_id'];?>##<?php echo $deviceData[$x]['itgc_id'];?>##<?php echo $deviceData[$x]['device_imei'];?>##<?php echo $deviceData[$x]['is_repaired'];?>##<?php echo $deviceData[$x]['is_cracked'];?>##<?php echo $deviceData[$x]['is_ffc'];?>" onClick="setRow('<?php echo $y; ?>');" class="checkbox1"></td>
			  <td><?php echo $y;?></td>
			...
			<?php $receive_from=$deviceData[$x]['Branch_Send'];
			  ...
			  <?php $dt=date('d-m-Y',strtotime($deviceData[$x]['closecase_date'])); 
		      if($dt=='01-01-1970' )
		      {
		       $dt='';
		      }
		      ...
		      {
		        $dt=date('d-m-Y H:i:s',strtotime($deviceData[$x]['closecase_date']));
		     }
		      ?>
		     <td><?php echo $dt ?></td>
...
<script type="text/javascript">
 var $dispatch = jQuery.noConflict()
  $dispatch(window).load(function() { $dispatch("#loadingmessage").fadeOut("slow"); })
</script>
<script>
function dispatchDevice() 
{
	...
	if(branch_name==0)
	{
		...
		document.dispatch_device.ddlBranchName.focus();
		return false;
	} 
	if(medium==0 )
	{
		alert("Please Select Dispatch Medium");
		document.dispatch_device.ddlMedium.focus();
		return false;
	}  
}
</script>
