...
if (isset($_SESSION['branch_id']) &&  !empty($_SESSION['branch_id'])) { 
  ...
  if (isset($_GET['deviceid']) &&  !empty($_GET['deviceid'])) {
    ...
    if (isset($_POST['submit'])) {
	  ...
	  $newphoneno = $_POST['phno']; //New Phone No

      if($deviceproblem == 11 || $deviceproblem == 12 || $deviceproblem == 13 || $deviceproblem == 14 || $deviceproblem == 32)
      {
          ...
          //echo "update mobile_simcards set mobile_no='".$phone_no[1]."' where id in (select sys_simcard from devices where devices.imei='".$deviceimei."')";die();
          $strSqlQuery =db__select_matrix("update mobile_simcards set mobile_no='".$phone_no[1]."' where id in (select sys_simcard from devices where devices.imei='".$deviceimei."')");
...
<body>
  ...
  <option value="Device Without Sim">Device Without Sim</option>
               ...
               <!-- <tr id="selected12" style="display: none">
                  <td>Old Phone No</td>
                  <td><input type="text" class="form-control" id="contactnoid"></td>
                </tr>-->
                ...
                <tr id="selected13" style="display: none">
                  <?php 
                    for($j=0;$j<count($dev_searchid);$j++){
                      $select .= '<option value="'.$dev_searchid[$j]['sim_id'].'">'.$dev_searchid[$j]['phone_no'].'</option>';
                    }
                  ?>    
                  <td>New Phone No</td> 
                  <td>  
  ...
  var $closeReapairCase = jQuery.noConflict();
    ...
    $closeReapairCase(document).ready(function(){
      ...
      $closeReapairCase('#problemsid').change(function(){
          ...
          //alert(id)
          if(id == '11' || id == '12'|| id == '13' || id == '14' || id =='32'){
