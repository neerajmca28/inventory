...
if (isset($_SESSION['userId_inv']) &&  !empty($_SESSION['userId_inv'])) 
{
  ...
  $veh_reg=$data[0]['phone_no'];
 // 28.635339737 ,77.139625549
  //echo "<pre>";print_r($veh_reg);
$testDevice1="999999";
  ...
  if(count($testDevice)>0)
	{
     $testDevice1=$testDevice[0]['radius'];
	}
   ...
   {    
     for ($i = 0; $i < count($_POST['rowVal']); $i++) 
     {
        ...
        //echo $checkBox[0];
         $devicestatus = $Tested;
          //echo "CALL SetTested('".$checkBox[0]."','".$devicestatus."')"; die;
...
<article>
  ...
  <?php 
  //echo round($testDevice1);
    if (round($testDevice1)<=500)
   {
	    //echo round($testDevice1);
