		...
		for($i=0;$i<$rowcount;$i++)
		{
			if(isset($_POST['rowVal'][$i]))
			{
				 ...
				 {	//echo $temp_ffc; die;	
					...
					//echo "CALL UpdateDispathManufactureDevice1('".$DeviceId."','".$deviceStatus1."','".$imei."')"; die;	 
				 	//$deviceStatus1=$FinalAttachSim; 
				 	$UpdateDispathNewDevice1=select_Procedure("CALL UpdateDispathManufactureDevice1('".$DeviceId."','".$deviceStatus1."','".$imei."')");			
				}
			}
...
<body>
 <form name="dispatchdevice" id="dispatchdevice" method="post" action="" >
	 ...
	 <div class="cl-item"><span class="lightblue"></span><span>Aqua :</span><span>Reparied Device</span></div>
	  ...
	  <div class="cl-item"><span class="aqua"></span><span>Manufacture Device</span></div>
	   </div>	
        ...
        <table class="table table-bordered table" id="filtertable">
          ...
          <tbody>
            ...
            <tr bgcolor="<?php echo $color; ?>" title="<?php echo $tool_tip; ?>">
              <td><input type="checkbox" id="check<?php echo $y; ?>" name="rowVal[]" value="<?php echo $SelectRepairOk[$x]['device_id'];?>##<?php echo $SelectRepairOk[$x]['device_imei'];?>##<?php echo $SelectRepairOk[$x]['device_status'];?>##<?php echo $SelectRepairOk[$x]['is_ffc'];?>##<?php echo $SelectRepairOk[$x]['is_permanent'];?>" onClick="setRow('<?php echo $y; ?>');" class="checkbox1"></td>
			  <td><?php echo $y;?></td>
			   ...
			   <td><?php echo $SelectRepairOk[$x]['client_name']; ?></td>
				...
				if($SelectRepairOk[$x]['device_status']==194)
				 ...
				 if($SelectRepairOk[$x]['device_status']==196)
				 {
				 	 	$dev_status="Manufacture Device";
				 }
				 ?>
