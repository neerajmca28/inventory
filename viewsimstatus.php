...
if(isset($_POST['simStatusBtn']))
{
    ...
    else 
    {
      ...
      //print_r($sim_statusid);
      $installer_view=select_Procedure("CALL proc_sim_viewInstaller('".$sim_status."')");
            ...
            <div class="content-box" id="simFilter" style="">
              <table class='table borderless'>
                ...
                <tr>
                  ...
                  <?php 
                    for($i=0;$i<count($simcount);$i++){
                      echo date('d-m-Y H:i:s',strtotime($simcount[$i]['rec_date']));
                    }
                  ?>
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td class="s">
                    ...
                    for($i=0;$i<count($simcount);$i++)
					{
						...
						{  
							if($simcount[$i]['installerID'] == 0 || $simcount[$i]['installerID'] == "")
							{

							$rep_name= mysql_query("select name from inventory.repair_user where id='".$sim_statusid[$i]['RepairName']."'");
							$tt=mysql_fetch_array($rep_name); 
							//print_r($tt);die;
							print_r($tt[0]);
							...
							{
								$strSqlQuery =db__select_staging("select installer.inst_name from internalsoftware.installer where inst_id='". $simcount[$i]['installerID']."'");
								echo  $installerName = $strSqlQuery[0]['inst_name'];  
							}  
						}
					   else
					   {
							//echo "select installer.inst_name from internalsoftware.installer where inst_id='". $simcount[$i]['installerID']."'"; die;
							$strSqlQuery =db__select_staging("select installer.inst_name from internalsoftware.installer where inst_id='". $simcount[$i]['installerID']."'");
						   echo  $installerName = $strSqlQuery[0]['inst_name']; 
					   }
